...
public class FabricPlatform implements Platform {
	...
	public MenuType<ToolPouchMenu> constructMenu() {
		return new ExtendedScreenHandlerType<>(ToolPouchMenu::new, ModPayloads.S2CToolPouchScreenPayload.CODEC);
	}
	...
	public InteractionResult openToolPouchScreen(Player player, ItemStack toolPouch) {
		if (!player.level().isClientSide()) {
			player.openMenu(new ExtendedScreenHandlerFactory<ModPayloads.S2CToolPouchScreenPayload>() {
				@Override
